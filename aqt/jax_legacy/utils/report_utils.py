...
def save_report(report: ExperimentReport,
  ...
  """
  model_dir = pathlib.Path(report.model_dir)
  report_path = '--'.join(list(model_dir.parts)[1:])
  report_path = os.path.splitext(report_path)[0]
  report_path = os.path.join(report_dir, report_path)

  if not os.path.exists(report_path):
    os.makedirs(report_path)
  if report.hparams_config_path is not None:
    hparams_config_path = os.path.join(report_path, HPARAMS_CONFIG_FILENAME)
    shutil.copyfile(report.hparams_config_path, hparams_config_path)
    report.hparams_config_path = hparams_config_path
  data_dict = dataclasses.asdict(report)
  path = os.path.join(report_path, REPORT_FILENAME)
